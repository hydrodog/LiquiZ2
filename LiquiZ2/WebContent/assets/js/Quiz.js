...
Quiz.prototype.makeEditBox = function (id) {
  ...
  var editFunc = function (e) {
    ...
    var ques = document.getElementById("qc" + arrayid[0]);
    console.log(ques, arrayid[0]);
    if (ques)
    ...
    console.log(q, q.level);
    editor.editOldQuestion(edi, q, num, arrayid[0]);
...
Quiz.prototype.text2equation = function (txt) {
  var div = null;
  if (typeof txt == 'string' || !txt) {
    div = Util.pre("`" + txt + "`");
  } else {
    div = VarWrittenParser(txt, 'div', 'text2equation');
