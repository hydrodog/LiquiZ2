...
public class Numeric extends Question implements java.io.Serializable{
	boolean caseSensitive;
	
	public Numeric(int id, int points, int level, ArrayList<Answer> answers) {
		super(id, points, level, answers);
	}
	
	// the new constructor including all of the elements
	public Numeric(int id, int points, int level, Answer answer){
		super(id, points, level, answer);
	}

	// the new constructor including all of the elements
	public Numeric(int points, int level, Answer answer){
		super(points, level, answer);
	}
	
	public Numeric(int id, int points, int level) {
	...
	public void writeHTML(DisplayContext dc) {
		if (dc.isDisplayResponses()) {
			...
			dc.append("<input name='").append(getId()).append("' class='numeric' type='text' />");
		}
	}

	@Override
	public void writeJS(DisplayContext dc) {
		dc.append(",\n\t\t\t[\"numeric\", ").append(getId()).append("]");
	}
